#' DIRECTIONS: 
#' 
#' 1. Source this file and invoke newYaml(lessonName, courseName), where
#' lessonName and courseName are strings of your choice such as "My Lesson 1" and 
#' "My Course". Spaces are allowed in course and lesson names.
#' This will create a file such as My_Course/My_Lesson_1/lesson.yaml.
#' The file may appear in the editor automatically. If not, open it manually.
#' 
#' 2. Fill in the course meta-information, Author, Organization, etc. as 
#' indicated in lesson.yaml. 
#' 
#' 3. Append instructional units such as multiple-choice questions to 
#' lesson.yaml by invoking appropriate helper functions. Type hlp() for a 
#' brief list. Typing qmult(), for instance would append the following 
#' template to lesson.yaml:
#' 
#' - Class: mult_question  
...
#' 
#' - Class: mult_question  
#'   Output: This demonstrates a multiple choice question. Which Scooby-Doo
#'      character wears an ascot?
...
#' 
#' 5. Strings may take more than one line provided they are surrounded by 
#' double quotes. For instance, in the following question the Output: field 
#' takes up two lines:
#' 
#' - Class: text_question
...
lessonFileCheck <- function(){
  while(!exists("lessonFile", globalenv()) || !file.exists(lessonFile)){
    ...
    lessonFile <<- file.path(course, lesson, "lesson.yaml")
  }
}